											   ...
											   haclog_meta_info_t *meta)
{
	...
	if (handler->enable_color && meta->loc->level >= HACLOG_LEVEL_WARNING) {
		...
		} else if (meta->loc->level >= HACLOG_LEVEL_INFO) {
			SetConsoleTextAttribute(console_handle,
		...
		if (meta->loc->level >= HACLOG_LEVEL_FATAL) {
			...
			fwrite(UNIX_TERMINAL_COLOR_GRN, 1, strlen(UNIX_TERMINAL_COLOR_GRN),
				   handler->fp);
		} else {
			fwrite(UNIX_TERMINAL_COLOR_CYN, 1, strlen(UNIX_TERMINAL_COLOR_CYN),
				   handler->fp);
		}
#endif
	} else {
